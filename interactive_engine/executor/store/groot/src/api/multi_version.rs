...
pub trait GraphUpdate: Send + Sync {
    fn insert_vertex(
        &self, si: SnapshotId, label: LabelId, schema_version: i32, id: VertexId,
    ...
    fn insert_edge(
        &self, si: SnapshotId, relation: Relation, schema_version: i32, edge_id: EdgeId, src_id: VertexId,
        dst_id: VertexId, data: &HashMap<i32, Vec<u8>>,
    );
    ...
    fn delete_edge(
        &self, si: SnapshotId, relation: Relation, edge_id: EdgeId, src_id: VertexId, dst_id: VertexId,
    );
...
pub trait GraphLoader: Send + Sync {
    fn load_table(
        &self, schema_version: SchemaVersion, table: TableInfo, source_data_dir: String,
    ) -> Result<(), String>;
...
pub trait MVGraph: MVGraphQuery + GraphUpdate + GraphLoader + DDL {
    ...
    ) -> Option<Arc<dyn MVGraphQuery<V = Self::V, E = Self::E, VI = Self::VI, EI = Self::EI>>>;
    fn get_dimension_partition(
        &self,
    ) -> Arc<dyn MVGraphQuery<V = Self::V, E = Self::E, VI = Self::VI, EI = Self::EI>>;
    fn clear(&self);
