...
impl VertexTypeScan {
    pub fn new(
        storage: Arc<dyn ExternalStorage>, snapshot_id: SnapshotId, vertex_type_info: Arc<VertexTypeInfo>,
    ) -> Self {
        VertexTypeScan { storage, snapshot_id, vertex_type_info }
    }
}
                ...
                .filter_map(move |(raw_key, raw_val)| {
                    ...
                    match parse_vertex_key(key) {
                        Ok((vertex_id, ts)) => {
                            ...
                            match self
                                .vertex_type_info
                                .get_decoder(snapshot_id, codec_version)
                            {
                                Ok(decoder) => {
...
impl EdgeTypeScan {
    pub fn new(
        storage: Arc<dyn ExternalStorage>, snapshot_id: SnapshotId, edge_info: Arc<EdgeInfo>,
        vertex_id: Option<VertexId>, direction: EdgeDirection,
    ) -> Self {
...
impl EdgeKindScan {
    pub fn new(
        storage: Arc<dyn ExternalStorage>, snapshot_id: SnapshotId, edge_kind_info: Arc<EdgeKindInfo>,
        vertex_id: Option<VertexId>, direction: EdgeDirection,
    ) -> Self {
